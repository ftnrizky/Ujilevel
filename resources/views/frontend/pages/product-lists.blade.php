                        ...
                        <div class="shop-sidebar">
                            ...
                            <div class="single-widget category">
                                ...
                                <ul class="categor-list">
                                    @php
                                        $categories = Helper::getAllCategory(); // Ambil semua kategori
                                    @endphp
                                    @foreach ($categories as $category)
                                        <li>
                                            <a href="{{ route('product-list', ['category' => $category->id]) }}"
                                                class="{{ request()->category == $category->id ? 'active' : '' }}">
                                                {{ $category->title }}
                                                @if ($category->child_cat->count() > 0)
                                                    <span class="caret"></span>
                                                    <!-- Icon panah untuk kategori punya subkategori -->
                                                @endif
                                            </a>
                                            ...
                                            @if ($category->child_cat->count() > 0)
                                                <ul class="sub-category">
                                                    @foreach ($category->child_cat as $child)
                                                        <li>
                                                            <a href="{{ route('product-list', ['category' => $child->id]) }}"
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            // Toggle subkategori saat diklik
            $('.categor-list li a').click(function(e) {
                if ($(this).find('.caret').length > 0) {
                    e.preventDefault(); // Hentikan aksi default jika punya subkategori
            ...
            $('.categor-list li a').each(function() {
                if ($(this).hasClass('active')) {
                    ...
                    $(this).parents('.sub-category').show();
                }
            });
        });
    </script>
